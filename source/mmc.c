...
int CardInitCard()
{
    ...
    if (iResult==0)
    {
		...
		//Try opening mp3 file directly from sd card
		int hvol;
		FILE *fp;
		char buff[256];
		...
		fp = fopen("PHAT0:/text.txt", "r");
	    fscanf(fp, "%s", &buff[0]);
	    //puts(buff);
		
		LogMsg_P(LOG_INFO, PSTR("Buffey: %s"), buff[0]);
		...
		/*
		FILE *testFile;
		if(testFile = _fdopen("text.txt", "r" ) != NULL)
		{
			LogMsg_P(LOG_INFO, PSTR("MP3 file is read"));
			LogMsg_P(LOG_INFO, PSTR("Trying to play music file"));
			char *makkertje = malloc(512);
			char yolo = fgetc(testFile);
			LogMsg_P(LOG_INFO, PSTR("Text file output: %c"), fgetc(testFile));
			
			PlayFile(testFile);
		}
		else
		{
			LogMsg_P(LOG_INFO, PSTR("Mp3 file could not be read"));
		}
		*/
		
		/*
		if(testFile = _fdopen( "FAT:videoplayback.mp3", "rb" ) != NULL)
		{
			LogMsg_P(LOG_INFO, PSTR("MP3 file is read"));
			LogMsg_P(LOG_INFO, PSTR("Trying to play music file"));
			PlayFile(testFile);
